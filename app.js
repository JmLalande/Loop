...
function open(){return new Promise((res,rej)=>{
  ...
  r.onupgradeneeded=e=>{const d=e.target.result;
    ...
    if(!d.objectStoreNames.contains("freezes"))d.createObjectStore("freezes",{keyPath:"date"});};
...
/* ------------------------------------------------------------------ state */
let SESSIONS={}, FREEZES={}, META={k:"app",poolSeen:[],lastBreak:-1,sound:true,vol:1,firstDay:null,lastStreak:0,facts:null}, WEIGHTS=[];
...
/* Where the cycle stands is derived, never stored. A cursor kept in `meta` is
   one failed write away from pointing at the wrong workout forever, and there
   is no way to tell from the outside that it has drifted. Counting the sessions
   that happened cannot drift: the answer is rebuilt from the history every time.
   Freezes are deliberately absent from this count. They hold the streak, they
...
/* Which cycle's freezes are spendable right now. Not floor(): the rest days come
   ...
   to, and the budget flips the moment she starts the new one. The cap is
   unchanged: two freezes per five sessions. */
const budgetCycle=()=>Math.max(0,Math.ceil(sessionCount()/CYCLE.length)-1);
const freezesSpent=c=>Object.values(FREEZES).filter(f=>f.cycle===c).length;
const freezesLeft=()=>Math.max(0,FREEZES_PER_CYCLE-freezesSpent(budgetCycle()));

/* A day holds the chain if she trained or if she froze. Only training counts
   toward the number on screen, so the streak reads as sessions done and the
   milestone keyed to it stays true. A frozen day is a day the chain survived,
   not a day she earned. */
const held=k=>qualifies(SESSIONS[k])||!!FREEZES[k];
function computeStreak(){
  let n=0,d=new Date();
  if(!held(iso(d)))d.setDate(d.getDate()-1);
  while(held(iso(d))){if(qualifies(SESSIONS[iso(d)]))n++;d.setDate(d.getDate()-1);}
  return n;
}

/* The days between the last day she held and yesterday. Today is never in here:
   today is not missed until it is over. */
function openGap(){
  if(!sessionCount())return [];
  const gap=[],d=new Date();d.setDate(d.getDate()-1);
  while(gap.length<=FREEZES_PER_CYCLE+1&&!held(iso(d))){gap.push(iso(d));d.setDate(d.getDate()-1);}
  return held(iso(d))?gap:null;   /* null means the gap runs off the end, too old to cover */
}

async function spendFreeze(date,auto){
  const rec={date,cycle:budgetCycle(),auto:!!auto};
  FREEZES[date]=rec;await put("freezes",rec);
}

/* Nothing runs at midnight in a page that is closed, so a missed day is only
   discovered when she opens the app again. Cover the whole gap or none of it:
   spending both freezes on a five-day absence breaks the streak anyway and
   leaves her with nothing for the Tuesday after. */
async function autoCover(){
  const gap=openGap();
  if(!gap||!gap.length||gap.length>freezesLeft())return 0;
  for(const date of gap)await spendFreeze(date,true);
  return gap.length;
}
...
const LADDER="Ajoute des reps, ensuite descends en 3 secondes, ensuite une pause de 2 secondes en bas, ensuite la veste.";
...
function renderToday(){
  const now=new Date(), slot=nextSlot(), day=CYCLE[slot];
  $("todayDate").textContent=DOW[now.getDay()]+" · "+now.toLocaleDateString(undefined,{month:"long",day:"numeric"});
  $("todayName").textContent=day.name;
  $("cyclePos").textContent="Workout "+(slot+1)+" of "+CYCLE.length;
  ...
  if(onRamp()){
    ...
    or.textContent="Semaines faciles, "+daysSinceStart()+"/"+ONRAMP_DAYS+" · "+ONRAMP_NOTE;
  }else or.style.display="none";

  const nudge=progressionNudge(), nd=$("nudge");
  if(nudge){nd.style.display="block";
    nd.innerHTML="<b>"+nudge.short+" : haut de la fourchette deux fois de suite.</b> "+(M[nudge.m].next||LADDER);}
...
/* ---------------------------------------------------------------- routine */
/* No weekday headings any more. The cycle has no opinion about what day it is,
   so the only orientation worth giving is which workout is up next. */
function renderRoutine(){
  const up=nextSlot();
  $("routineList").innerHTML=CYCLE.map((d,slot)=>{
...
async function toggleFreeze(){
  ...
  if(FREEZES[today]){
    delete FREEZES[today];await del("freezes",today);
    toast("Freeze returned");
  }else{
    if(!freezesLeft())return;
    await spendFreeze(today,false);
    toast("Today is frozen. The streak holds.");
  }
...
function renderHistory(){
  ...
  $("recent").innerHTML=list.length?list.map(s=>{
    ...
    return "<div class='stat' data-edit='"+s.date+"' style='cursor:pointer'><span>"+
      ...
      " · "+(CYCLE[s.dayKey]?CYCLE[s.dayKey].name:"Session")+"<br><span class='muted'>"+n+(n===1?" set · ":" sets · ")+t+"</span></span>"+
...
function render(){
  ...
  /* A rep target that already carries its own unit says so with a space in it,
     like "15 aller, 15 retour". Appending "reps" to that reads as gibberish. */
  const unit=sm.side?" "+sm.side:((sm.hold||/\s/.test(show.reps))?"":" reps");
  $("target").innerHTML="<b>"+show.reps+"</b>"+unit+" · <b>"+sm.load+"</b>";
  $("effort").innerHTML=show.rir
    ?(onRamp()?"Semaines faciles · ":"")+"arrête <b>"+rirLbl(show.rir)+"</b> reps avant l'échec"
    :"";
...
const SIDES=[{k:"R",lbl:"Droite"},{k:"L",lbl:"Gauche"}];
let sideVal=null;
...
function openSingle(m){
  ...
  $("rirLbl").textContent=pendingWork.rir
    ?"Reps en réserve · vise "+rirLbl(pendingWork.rir)
    :"Reps en réserve";
...
function openSides(m){
  ...
  lbl.textContent=pendingWork.rir
    ?"Un côté à la fois · vise "+rirLbl(pendingWork.rir)+" reps en réserve"
    :"Un côté à la fois";
...
async function saveSession(complete,actual){
  ...
  if(FREEZES[date]){delete FREEZES[date];await del("freezes",date);}
...
function leaveSession(){
  ...
  /* The cursor moved, so the Next marker on the routine page is now pointing at
     the workout that was just finished. It only ever rendered once, at load. */
...
function openEditor(date){
  ...
  const eday=CYCLE[s.dayKey]||CYCLE[0];
  $("edDay").textContent=eday.name;
  $("edMove").innerHTML=eday.blocks.map(b=>"<option value='"+b.m+"'>"+M[b.m].name+"</option>").join("");
  renderEdList();
...
function renderEdList(){
  ...
  h+=s.sets.map((x,i)=>{
    ...
    const sd=x.side==="L"?" · Gauche":(x.side==="R"?" · Droite":"");
...
const dump=()=>JSON.stringify({sessions:Object.values(SESSIONS),meta:META,weights:WEIGHTS,freezes:Object.values(FREEZES)},null,2);
...
async function ingest(d){
  ...
  for(const f of (d.freezes||[]))await put("freezes",f);
...
async function load(){
  ...
  const fz=await all("freezes");FREEZES={};fz.forEach(x=>FREEZES[x.date]=x);
  /* Before the first render, or she sees a broken streak that the freezes she
     is owed were about to save. */
  const covered=await autoCover();
  META.lastStreak=computeStreak();await put("meta",META);
  $("verNote").textContent=APP_VERSION;
  renderToday();renderRoutine();renderHistory();renderStats();
  if(covered)toast(covered===1?"A freeze covered yesterday":covered+" freezes covered the days you missed");
}
