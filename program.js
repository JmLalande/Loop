...
const M = {
  ...
  stepup:   {short:"Step-up",       name:"Step-up",                            reps:"8–12",  lo:8,  hi:12, side:"chaque jambe", load:"poids du corps", work:75, def:10, next:"Rien pour l'instant. Celui-là est en probation. Deux cycles complets sans douleur au genou avant de monter sur une marche plus haute, et la veste attend après ça.", cue:"Une marche d'escalier, ou un step solide qui glisse pas. Le pied au complet sur la marche, pas juste les orteils. Penche le torse un peu vers l'avant et pousse dans le talon du pied du haut pour monter. La jambe du bas pousse pas, elle suit. Le genou reste enligné avec les orteils, il rentre pas vers l'intérieur. Redescends lentement, sans te laisser tomber. Fais toutes les reps d'une jambe, ensuite change. Une main sur le mur ou sur la rampe, c'est correct. Si le genou se plaint, prends une marche plus basse, même si c'est bas."},
  /* Parked. Every version of it hurt her knee, with a deficit or without. The step-up replaced it. */
  ...
  /* Parked. Day 3 needed glute volume more than a second knee hold, and the wall sit on day 1 already covers the isometric. It is also the lunge position, and every lunge hurt her knee, so it only comes back after a set that does not hurt. */
...
/* Reps left in the tank at the end of a set. Lower is harder. The step-up overrides its own day and runs softer, because that one movement is the one that loads the bad knee. */
...
/* The cycle. Five workouts in a fixed order and a cursor that moves only when a
   ...
   Within a day, block order is execution order. Hardest and most technical first, core last. Day 3 runs the step-up second on purpose: it is the highest-risk movement in the program and it should not land on tired legs. */
const CYCLE = [
  ...
  {name:"Legs & glutes",rir:[1,2], blocks:[{m:"rdl",sets:3,rest:42,reps:"8–10",lo:8,hi:10},{m:"stepup",sets:3,rest:45,rir:[2,3]},{m:"kickback",sets:3,rest:40}]},
